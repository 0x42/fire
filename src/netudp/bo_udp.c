...
int bo_send_udp_packet(int socket, unsigned char *packet, int len, char *ip)
{
    ...
    if (sendto(socket, (void *)packet, len, 0, (struct sockaddr *) &saddr, sizeof(saddr)) == -1)
	    ans = -1;
    
    return ans;
}

int bo_recv_udp_packet(void *buf, int bufSize, int socket, char **sender_host, int *sender_port)
{
    struct sockaddr_in saddr;
